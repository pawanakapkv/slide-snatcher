...
with st.sidebar:
    ...
    2. Download `cookies.txt` from YouTube.
    3. Upload it below.
    """)
    ...
    if not url:
        ...
        with st.spinner("Fetching video formats..."):
            ...
            if info:
                ...
                st.error("Could not find video. Try uploading cookies in the sidebar.")
...
if st.session_state['video_info'] and url == st.session_state['url_input']:
    ...
    unique_heights = set()
    for f in formats:
        if f.get('vcodec') != 'none' and f.get('height'): unique_heights.add(f['height'])
    sorted_heights = sorted(unique_heights, reverse=True)
    
    quality_options = {}
    for h in sorted_heights: quality_options[f"{h}p (Stream)"] = f"bestvideo[height={h}][protocol^=http]"
    ...
    if start_btn:
        ...
        status_text.info(f"🌐 Fetching stream URL...")
        stream_url, meta_fps = get_stream_url(url, selected_format_str, cookies_path)
